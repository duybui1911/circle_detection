...
from circle.MathFunc.Point3 import Point3
from circle.MathFunc.Vector2 import Vector2
from circle.MathFunc.Vector3 import Vector3
from circle.MathFunc.Constants import Constants
...
class MathUtils:
    ...
    def ProjectToPlane(point, planeNormal, planePoint):
        ...
        return Point3(
            (point.x - (normal.i * distance)), 
            (point.y - (normal.j * distance)), 
            (point.z - (normal.k * distance))
        )
    ...
    def areSame(v1, v2):
        return (
            MathUtils.areSame(v1.i, v2.i) 
            and MathUtils.areSame(v1.j, v2.j) 
            and MathUtils.areSame(v1.k, v2.k)
        )
    ...
    def Check_Intersects_LineSegmentLineSegment(p1, p2, p3, p4):
        ...
        if (MathUtils.EQTF(denominator, 0.0, tolerance)):
            if(
                MathUtils.EQTF(numeratorB, 0.0, tolerance) 
        ...
        if(
            (ua > (0.0 - Pixeltolerance)) 
            and (ua < (1.0 + Pixeltolerance)) 
    ...
    def Intersects_LineSegmentLineSegment(p1, p2, p3, p4):
        ...
        if(
            (ua > (0.0 - tolerance)) 
            and (ua < (1.0 + tolerance)) 
            and (ub > (0.0 - tolerance)) 
            and (ub < (1.0 + tolerance))
        ):
            intersection = p1 + (p2 -p1) * ua
